...
<html>
    <head>
        <title>Stock Return #{{ $data->return_number }}</title>
    </head>
    <body>
        
        <table>
            <tr>
                <td><h2>STOCK RETURN</h2></td>
            </tr>
        </table>
        ...
        <div class='mb-1rem'>
            <table class='w-100'>
                <tr>
                    <td class='w-50'>
                        <div class='display-block'>Stock Return Number: {{ $data->return_number }}</div>
                        <div class='display-block'>Stock Return Date: {{ $data->return_date }}</div> 
                ...
                <td class='v-top w-50 pr-20px'>
                    <div class='bold display-block'>Bill From </div>
                        ...
                        @if ($data->store->vat_number != '')
                            <div>GST: {{ $data->store->vat_number }}</div>
                        @endif
        ...
        <div class="mb-1rem">
            <table class="w-100 product-table mb-1rem">
                ...
                <tbody>
                    ...
                    @foreach ($data->products as $item_key => $stock_return_products)
                    <tr v-for="(po_product, key, index) in products" v-bind:value="$stock_return_products->product_slack" v-bind:key="index">
                        <td>{{ $item_key+1 }}</td>
                        <td>{{ ($stock_return_products->product_code != ''? $stock_return_products->product_code." - ": '') }}{{ $stock_return_products->name }}</td>
                        <td class="right">{{ $stock_return_products->quantity }}</td>
                        <td class="right">{{ $stock_return_products->amount_excluding_tax }}</td>
                        <td class="right">{{ $stock_return_products->discount_amount }}<br>({{ $stock_return_products->discount_percentage }}%)</td>
                        
                        @if (isset($data->tax_option_data) && count($data->tax_option_data->component_array) > 0)
                            @foreach ($data->tax_option_data->component_array as $component_array_key => $component_array_item)
                                <td class="right">{{ $stock_return_products->tax_component_array->$component_array_item }}</td>
                            @endforeach
                        @else
                            <td class="right">{{ $stock_return_products->tax_amount }}<br>({{ $stock_return_products->tax_percentage }}%)</td>
                        @endif
                        
                        <td class="right">{{ $stock_return_products->subtotal_amount_excluding_tax }}</td>
                    </tr>
                    @endforeach
