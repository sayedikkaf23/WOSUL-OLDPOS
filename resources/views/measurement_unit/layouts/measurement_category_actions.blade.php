<div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown">
        @if (check_access(array('A_DETAIL_MEASUREMENT_UNIT'), true))
            <a href="{{ $measurement_category['detail_link'] }}" class="dropdown-item">{{ __("View") }}</a>
        @endif
        @if (check_access(array('A_EDIT_CATEGORY'), true))
            <a href="edit_measurement_category/{{ $measurement_category['slack'] }}" class="dropdown-item">{{ __("Edit") }}</a>
        @endif
    </div>
</div>