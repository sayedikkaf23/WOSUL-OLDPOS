    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="d-flex flex-wrap mb-4">
                ...
                <div class="">
                    <span v-bind:class="po_basic.status.color">{{ $t(po_basic.status.label) }}</span>
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap mb-4" v-if="po_statuses != ''">
                ...
                <div class="ml-auto">
                   ...
                   <!--  <button type="button" class="btn btn-outline-primary mr-1"  v-if="create_invoice_from_po_access == true" v-on:click="generate_invoice()" v-bind:disabled="generate_invoice_processing == true"> <i class='fa fa-circle-notch fa-spin' v-if="generate_invoice_processing == true"></i> {{ $t("Generate Invoice") }}</button> -->
                    ...
                    <button type="submit" class="btn btn-danger mr-1" v-if="delete_po_access == true" v-on:click="delete_po()" v-bind:disabled="po_delete_processing == true"> <i class='fa fa-circle-notch fa-spin'  v-if="po_delete_processing == true"></i> {{ $t("Delete Purchase Order") }}</button>

                    <div class="dropdown d-inline" v-if="purchase_order_data.status.value != 4 ">
            ...
            <div class="table-responsive">
                <table class="table table-striped display nowrap text-nowrap w-100">
                    <thead>
                        ...
                        <th scope="col" class="text-right">{{ $t("Total Amount") }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(po_product, key, index) in products" v-bind:value="po_product.product_slack" v-bind:key="index">
                            ...
                            <td class="text-right">{{ po_product.total_amount }}</td>
                        </tr>
                        <tr>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            console.log(this.purchase_order_data.status.value,'order status');
        },
        methods: {
