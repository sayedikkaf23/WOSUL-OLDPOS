      ...
      <div class="d-flex flex-wrap mb-4">
        <div class="mr-auto">
          <span class="text-title">
            {{ $t("Bonat User Points Settings") }}</span
          >
        </div>
      </div>
      <form @submit.prevent="submit_form" class="mb-3">
        <div class="form-row mb-2">
          <div class="form-group col-md-2">
            <label for="bonat_merchant_id">{{ $t("Bonat Merchant Id") }}</label>
            <p class="text-truncate">{{ bonat_merchant_id }}</p>
          </div>
         
                   <div class="form-group col-md-2">
            <label for="status">{{ $t("Store Slack") }}</label>
            <p>
              <span v-bind:class="status_color">{{ store_slack }}</span>
            </p>
          </div>
                   <div class="form-group col-md-2">
            <label for="status">{{ $t("Counter Slack") }}</label>
            <p>
              <span v-bind:class="status_color">{{ counter_slack }}</span>
            </p>
          </div>
          <div class="form-group col-md-2">
...
export default {
  data() {
    return {
      ...
      verify_merchant: "/api/verify_bonat_merchant_setting",
      edit_link:"/edit_bonat_user_points_setting/" + this.bonat_setting.slack,
        
      bonat_merchant_id:
        this.bonat_setting.length == 0
          ? "-"
          : this.bonat_setting.bonat_merchant_id,
      status_label:
  ...
  methods: {
    submit_form() {
      ...
      formData.append(
        "bonat_merchant_id",
        this.bonat_setting.bonat_merchant_id == null
          ? ""
          : this.bonat_setting.bonat_merchant_id
      );
