        ...
        <div class="d-flex flex-wrap mb-4">
          <div class="mr-auto">
            <span class="text-title" v-if="setting_slack == ''"
              >{{ $t("Add Bonat User Points Setting") }}
            </span>
            <span class="text-title" v-else
              >{{ $t("Edit Bonat User Points Setting") }}
            </span>
          </div>
          <div class="">
            <button
    ...
    return {
      ...
      api_link:
        this.setting_data.length == 0
          ? "/api/add_setting_bonat_user_points"
          : "/api/update_setting_bonat_user_points/" + this.setting_data.slack,

      setting_slack:
        this.setting_data.length == 0 ? "" : this.setting_data.slack,
      bonat_merchant_id: this.setting_data.length == 0 ? "" : this.setting_data.bonat_merchant_id,
      status:
      ...
      this.$validator.validateAll().then((result) => {
        if (result) {
          ...
          this.$on("submit", function() {
            ...
            formData.append(
              "bonat_merchant_id",
              this.bonat_merchant_id == null ? "" : this.bonat_merchant_id
            );
