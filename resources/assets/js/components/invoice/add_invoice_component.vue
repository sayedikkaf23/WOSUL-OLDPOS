    ...
    <div class="col-md-12">
      <form @submit.prevent="submit_form" class="mb-3">
        <div class="d-flex flex-wrap mb-4">
          <div class="mr-auto">
            <span class="text-title" v-if="invoice_slack == ''">{{
              $t("Add Invoice")
            }}</span>
        ...
        <div class="form-row mb-2">
          <div class="form-group col-md-3">
            ...
            <select
              ...
              @change="loadSupplierCustomer(bill_to),loadCategories(bill_to)"
            >
          ...
          <div class="form-group col-md-3">
            <label for="bill_to_slack"
              > <span v-if="bill_to == 'CUSTOMER' || bill_to == '' ">{{ $t("Choose Customer") }}</span> <span v-else>{{ $t("Choose Company") }}</span>  *<button
                type="button"
                class="btn btn-sm btn-secondary"
                @click="addSupplier()"
                v-if="bill_to == 'SUPPLIER' || bill_to == 'COMPANY'"
              >
                +
              </button>
              <button
            ...
            <Select2
                ...
                @change="loadCategories('CUSTOMER')"
                name="bill_to_slack"
            />
<!--            <select
              class="form-control"
              ...
              @change="loadCategories('CUSTOMER')"
            >
              <option v-if="bill_to == 'SUPPLIER' || 'COMPANY'" value="WriteCustomer">{{ $t("Enter Customer Name") }}</option>
              <option v-if="bill_to == 'SUPPLIER' || 'COMPANY'" value="DEFAULT">{{
                $t("DEFAULT")
              }}</option>
              <option
          ...
          <div class="form-group col-md-3" v-if="write_customer == true">
            <label for="bill_to_slack">Enter Customer Name</label>
          <input
              type="text"
              name="write_customer_name"
              v-model="write_customer_name"
              v-if="write_customer == true" placeholder="Enter Customer Name"
        ...
        <div class="form-row mb-2">
          <div class="form-group col-md-3">
            ...
            <input
              ...
              autocomplete="off"
            />
            <span v-bind:class="{ error: errors.has('invoice_date') }">{{
          ...
          <div class="form-group col-md-3">
            ...
            <input
              ...
              autocomplete="off"
            />
            <span v-bind:class="{ error: errors.has('invoice_due_date') }">{{
        ...
        <div class="" v-if="show_product_list == true || invoice_slack != ''">
          <div class="form-row">
          ...
          >
            <div class="form-group col-md-2">
              <input
                type="text"
                v-bind:name="'item.name_' + index"
                v-model="item.name"
                v-validate="item.product_type == 2 ? 'required|max:50' : 'required' "
                :data-vv-as="product_name"
                class="form-control form-control-custom"
                autocomplete="off"
                :readonly="item.product_type == 1"
              />
              <span
                v-bind:class="{ error: errors.has('item.name_' + index) }"
                >{{ errors.first("item.name_" + index) }}</span
              >
            </div>
             <div class="form-group col-md-2">
              <input
                type="text"
                v-bind:name="'item.description_' + index"
                v-validate="item.product_type == 2 ? 'max:250' : '' "
              ...
              />
              <span
                v-bind:class="{ error: errors.has('item.description_' + index) }"
                >{{ errors.first("item.description_" + index) }}</span
              >
            </div>
            <div class="form-group col-md-1">
              <input
                type="number"
                v-bind:name="'item.quantity_' + index"
                ...
                v-on:input="
                  calculate_price();
                  check_product_quantity($event, item.slack);
                "
              />
              <span
                v-bind:class="{
                  error: errors.has('item.quantity_' + index),
                }"
                >{{ errors.first("item.quantity_" + index) }}</span
              >
            </div>
            <div class="form-group col-md-1">
              <input
                type="number"
                v-bind:name="'item.unit_price_' + index"
              ...
              <span
                v-bind:class="{
                  error: errors.has('item.unit_price_' + index),
                }"
                >{{ errors.first("item.unit_price_" + index) }}</span
              >
            </div>
            <div class="form-group col-md-1">
              <select
                  ...
                  autocomplete="off"
                  @change="changeDiscountType(item.discount_type)"
                  v-bind:name="'item.discount_type_' + index"
              >
                <option value="0">{{ $t("Choose Discount Type..") }}</option>
                <option value="1" :key="1">{{ $t("Amount") }}</option>
                <option value="2" :key="2">{{ $t("Percentage(%)") }}</option>
              </select>
            </div>
            <div class="form-group col-md-1">
              <input
                type="number"
                v-bind:name="'item.discount_percentage_' + index"
              ...
              <span
                v-bind:class="{
                  error: errors.has('item.discount_percentage_' + index),
                }"
                >{{
                  errors.first("item.discount_percentage_" + index)
                }}</span
              >
            </div>
            <div class="form-group col-md-1" v-if="item.is_taxable == 0">
              <input
                type="text"
                disabled=""
                readonly=""
            ...
            <div class="form-group col-md-1" v-if="item.is_taxable == 1">
              <select
                class="form-control"
                v-model="item.tax_code"
                v-bind:name="'item.tax_code_' + index"
                placeholder="Please choose tax code"
                autocomplete="off"
                @change="setTaxPercentage(item.tax_code, index)"
              >
            ...
            <div class="form-group col-md-1">
              <input
                type="number"
                v-bind:name="'item.tax_percentage_' + index"
              ...
              <span
                v-bind:class="{
                  error: errors.has('item.tax_percentage_' + index),
                }"
                >{{ errors.first("item.tax_percentage_" + index) }}</span
              >
            </div>
            <div class="form-group col-md-1">
              <select
                v-bind:name="'item.measurement_id_' + index"
                v-model="item.measurement_id"
                v-validate="item.measurement_id != '' ? 'required' : ''"
                class="form-control form-control-custom custom-select"
                disabled
              >
                ...
                <option
                  v-for="(measurement,
                  measurement_index) in item.measurements"
              ...
              <span
                v-bind:class="{
                  error: errors.has('item.measurement_id_' + index),
                }"
                >{{ errors.first("item.measurement_id_" + index) }}</span
              >
            </div>
            <div class="form-group col-md-1">
              <input
                type="number"
                v-bind:name="'item.amount_' + index"
                v-model="item.amount"
                v-validate="'required|decimal|min_value:0'"
                data-vv-as="Amount"
                class="form-control form-control-custom"
              ...
              />
              <span
                v-bind:class="{ error: errors.has('item.amount_' + index) }"
                >{{ errors.first("item.amount_" + index) }}</span
              >
            </div>
        ...
        <div
          class="form-row mb-3"
          v-show="invoice_charges != null && invoice_charges.length"
          v-for="(charge, index) in invoice_charges"
          :key="'INV' + index"
        >
          <div class="col-md-2 offset-md-7 text-right">
...
export default {
    ...
    return {
      ...
      api_link:
        this.invoice_data == null
          ? "/api/add_invoice"
          : "/api/update_invoice/" + this.invoice_data.slack,
      ...
      bill_to:
        this.invoice_data == null ? '' : this.invoice_data.bill_to,
      bill_to_slack:
        this.invoice_data == null
          ? ""
          : this.invoice_data.bill_to == "COMPANY"
          ? this.invoice_data.supplier.slack
          : this.invoice_data.customer != null
          ? this.invoice_data.customer.slack
          : "",
      write_customer: false,
      bill_to_label:
        this.invoice_data == null ? "" : this.invoice_data.bill_to_name,

      invoice_slack: this.invoice_data == null ? "" : this.invoice_data.slack,
      invoice_date:
        this.invoice_data == null
          ? new Date().toISOString().substr(0, 10)
          : this.invoice_data.invoice_date != null
          ? this.invoice_data.invoice_date_raw
          : "",
      invoice_due_date:
        this.invoice_data == null
          ? new Date().toISOString().substr(0, 10)
          : this.invoice_data.invoice_due_date != null
          ? this.invoice_data.invoice_due_date_raw
          : "",
      currency:
        this.invoice_data == null
          ? this.session_currency_code
          : this.invoice_data.currency_code != null
          ? this.invoice_data.currency_code
          : "",
      tax_option:
        this.invoice_data == null
          ? 0
          : this.invoice_data.tax_option_id != null
          ? this.invoice_data.tax_option_id
          : "",
      ...
      terms:
        this.invoice_data == null
          ? this.store_data.invoice_policy_information
          : this.invoice_data.terms != null
          ? this.invoice_data.terms
          : "",
      grand_total: 0,
      ...
      invoice_product_list:
        this.invoice_data != null ? this.invoice_data.products : [],
      ...
      suppliers_or_customers:
        this.invoice_data != null
          ? this.loadSupplierCustomer(this.invoice_data.bill_to)
          : "",
      suppliers: [Array, Object],
      customers: [Array, Object],
      invoice_charges:
        this.invoice_data_charges == null ? "" : this.invoice_data_charges,
      store_tax_code_id:
      ...
      product_description: this.$t("Description"),
      service_name: this.$t("Name"),
      success: this.$t("SUCCESS"),
      non_taxable_label: this.$t("Non Taxable"),
      customer_options :this.invoice_data != null
          ? this.get_option_array(this.invoice_data.bill_to)
          : []
  ...
  props: {
    ...
    invoice_data_charges: [Array, Object],
  ...
  watch: {
    bill_to_slack: function(val) {
      if (val) {
        ...
        this.product_list = [];
        if (this.bill_to == "SUPPLIER" || "COMPANY") {
          this.update_product_list();
        }
        if(val == "WriteCustomer"){
          this.write_customer = true;
        }else{
          this.write_customer = false;
        }
  ...
  mounted() {
    ...
    // console.log(this.invoice_data.total_tax_amount);
    // console.log(this.invoice_tax_amount);
  ...
  created() {
    ...
    // if(this.invoice_data.supplier.slack){
    //     this.all_products = this.loadProducts(this.invoice_data.supplier.slack);
    // }
  },
  ...
  methods: {
    ...
    load_products(keywords) {
      if (typeof keywords != "undefined") {
        var supplier =
          this.bill_to == "SUPPLIER" || "COMPANY" ? this.bill_to_slack : this.supplier;
    ...
    calculate_discount(item_total, discount_type ,discount_percentage) {
      if(discount_type == 1){
        var discount_amount = parseInt(discount_percentage) + parseFloat(0.00);
      }else{
        var discount_amount =
    ...
    changeDiscountType(tax_code_id){
      this.calculate_price();
    },

    calculate_price() {
      var invoice_tax = $("#tax_option option:selected").attr("data-value");
      ...
      for (var index in this.products) {
        ...
        if (
          ...
          if (
            ...
            if (discount_percentage >= 0 && discount_type >= 0) {
      ...
      var total_charges = 0;
      if (this.invoice_charges.length) {
        this.invoice_charges.forEach(function(rs) {
          total_charges += parseFloat(rs.amount);
          grand_total = parseFloat(grand_total) + parseFloat(rs.amount);
        });
      }

      // if(invoice_tax == undefined && this.invoice_data != null){
      //     this.invoice_tax_amount = parseFloat(this.invoice_data.total_tax_amount);
      // }
      // else{
      //     this.invoice_tax_amount = (invoice_tax > 0) ? total_charges * invoice_tax / 100 : 0;
      // }

      this.invoice_tax_amount = grand_total_tax;
      ...
      this.$validator.validateAll().then((result) => {
        if (result) {
          ...
          this.$on("submit", function() {
            ...
            formData.append(
              "write_customer_name",
              this.write_customer_name != null ? this.write_customer_name : ""
            );
            formData.append(
              "invoice_date",
              this.convert_date_format(this.invoice_date)
            );
            formData.append(
              "invoice_due_date",
              this.convert_date_format(this.invoice_due_date)
            );
            formData.append(
              "currency",
            ...
            formData.append("invoice_tax_amount", this.invoice_tax_amount);
            formData.append("products", JSON.stringify(this.products));

            if (this.invoice_charges.length) {
              formData.append(
                "invoice_charges",
                JSON.stringify(this.invoice_charges)
              );
            }
            this.add_product_errors = "";
            axios
              ...
              .then((response) => {
                if (response.data.status_code == 200) {
                  ...
                  setTimeout(function() {
                    location.href = "/invoice/" + response.data.data;
                  }, 1000);
                } else {
    ...
    loadSupplierCustomer(bill_to) {
      ...
      // if (bill_to == "SUPPLIER" || bill_to == "COMPANY") {
      //   var formData = new FormData();
      //   formData.append("access_token", window.settings.access_token);
      ...
      //   });
        ...
        axios.post("/api/load_customers", formData).then((response) => {
          this.suppliers_or_customers = response.data;
          this.customer_options = this.get_option_array(response.data);
        });
      // }
    },
    ...
    addCharge() {
      ...
      this.invoice_charges.push(item);
    },
    removeCharge(index) {
      this.invoice_charges.splice(index, 1);
      this.calculate_price();
    },
    ...
    get_option_array(array){
      let option_array = [{id:'WriteCustomer',text:'Enter Customer Name'},{id:'DEFAULT',text:'DEFAULT'}];
      array.forEach(item => {
        option_array.push({id: item.slack, text: (item.name != '') ? item.name : (item.phone != '') ? item.phone : item.email});
      });
      return option_array;
    },
  },
