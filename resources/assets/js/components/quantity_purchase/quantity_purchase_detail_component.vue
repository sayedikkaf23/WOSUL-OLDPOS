<template>
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="d-flex flex-wrap mb-4">
                <div class="mr-auto">
                   <div class="d-flex">
                        <div>
                            <span class="text-title"> {{ $t("Quantity Purchase") }} #{{ qp_basic.po_number }} </span>
                        </div>
                    </div>
                </div>
                <div class="">
                    <span v-bind:class="qp_basic.status.color">{{ $t(qp_basic.status.label) }}</span>
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap mb-4" v-if="po_statuses != ''">
                ...
                <div class="ml-auto">
                    ...
                    <a class="btn btn-outline-primary mr-1" v-bind:href="'/print_quantity_purchase/'+slack" target="_blank">{{ $t("Print") }}</a>
                   ...
                   <!--  <button type="button" class="btn btn-outline-primary mr-1"  v-if="create_invoice_from_po_access == true" v-on:click="generate_invoice()" v-bind:disabled="generate_invoice_processing == true"> <i class='fa fa-circle-notch fa-spin' v-if="generate_invoice_processing == true"></i> {{ $t("Generate Invoice") }}</button> -->
                    ...
                    <button type="submit" class="btn btn-danger mr-1" v-on:click="delete_qp()" v-bind:disabled="po_delete_processing == true"> <i class='fa fa-circle-notch fa-spin'  v-if="po_delete_processing == true"></i> {{ $t("Delete Quantity Purchase") }}</button>
                    ...
                    <!-- <div class="dropdown d-inline">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="po_action">
                            <button class="dropdown-item" type="button" v-for="(po_status, key, index) in po_statuses" v-bind:value="po_status.value_constant" v-bind:key="index" v-on:click="change_po_status(po_status.value_constant)">{{ $t("Mark as") }} {{ $t(po_status.label) }}</button>
                        </div>
                    </div> -->
                </div>
            ...
            <div class="form-row mb-2">
                <div class="form-group col-md-3">
                    ...
                    <p>{{ qp_basic.po_reference }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="order_date">{{ $t("Order Date") }}</label>
                    <p>{{ qp_basic.order_date }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="order_due_date">{{ $t("Order Due Date") }}</label>
                    <p>{{ qp_basic.order_due_date }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="created_by">{{ $t("Created By") }}</label>
                    <p>{{ (qp_basic.created_by == null)?'-':qp_basic.created_by['fullname']+' ('+qp_basic.created_by['user_code']+')' }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="updated_by">{{ $t("Updated By") }}</label>
                    <p>{{ (qp_basic.updated_by == null)?'-':qp_basic.updated_by['fullname']+' ('+qp_basic.updated_by['user_code']+')' }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="created_on">{{ $t("Created On") }}</label>
                    <p>{{ qp_basic.created_at_label }}</p>
                </div>
                <div class="form-group col-md-3">
                    <label for="updated_on">{{ $t("Updated On") }}</label>
                    <p>{{ qp_basic.updated_at_label }}</p>
                </div>
                <div class="form-group col-md-3">
            ...
            <div class="mb-3">
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        ...
                        <p>{{ qp_basic.supplier_name }}</p>
                    </div>
                    <div class="form-group col-md-3">
                        <label for="supplier_address">{{ $t("Address") }}</label>
                        <p>{{ qp_basic.supplier_address }}</p>
                    </div>
                    <div class="form-group col-md-3">
                        <label for="currency_name">{{ $t("Currency") }}</label>
                        <p>{{ qp_basic.currency_name }} ({{ qp_basic.currency_code }})</p>
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped display nowrap text-nowrap w-100">
                    <thead>
                        ...
                        <th scope="col" class="text-right" v-show="tax_component_count > 0" v-for="(tax_component_array_item, index) in tax_component_array" v-bind:key="index">{{ $t("Tax") }} %</th>
                    ...
                    <tbody>
                        <tr v-for="(po_product, key, index) in products" v-bind:value="po_product.product_slack" v-bind:key="index">
                            ...
                            <td class="text-right" v-show="tax_component_count > 0" v-for="(tax_component_array_item, index) in tax_component_array" v-bind:key="index">{{ po_product.tax_percentage }}</td>
                            <td class="text-right">{{ po_product.tax_amount }}</td>
                            <td class="text-right">{{ po_product.subtotal_amount_excluding_tax }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Sub Total") }} ({{$t("EXCL Tax")}})</td>
                            <td class="text-right">{{ qp_basic.subtotal_excluding_tax }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Total Discount") }}</td>
                            <td class="text-right">{{ qp_basic.total_discount_amount }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Total After Discount") }}</td>
                            <td class="text-right">{{ qp_basic.total_after_discount }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Total Tax") }}</td>
                            <td class="text-right">{{ qp_basic.total_tax_amount }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Shipping Charge") }}</td>
                            <td class="text-right">{{ qp_basic.shipping_charge }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Packaging Charge") }}</td>
                            <td class="text-right">{{ qp_basic.packing_charge }}</td>
                        </tr>
                        <tr>
                            <td v-bind:colspan="table_colspan" class="text-right">{{ $t("Total") }} ({{$t("INCL Tax")}})</td>
                            <td class="text-right">{{ qp_basic.total_order_amount }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="form-row mb-2">
                <div class="form-group col-md-6">
                    <p class='custom-pre'>{{ (qp_basic.terms != null)?qp_basic.terms:'-' }}</p>
                </div>
            </div>
        </div>
        ...
        <modalcomponent v-if="show_generate_invoice_modal" v-on:close="show_generate_invoice_modal = false">
            ...
            <template v-slot:modal-body>
                <p>{{ $t("Invoice will be created from this purchase order") }}.</p>
                {{ $t("Are you sure you want to proceed?") }}
            </template>
            <template v-slot:modal-footer>
                <button type="button" class="btn btn-light" @click="$emit('close')">{{ $t("Cancel") }}</button>
    ...
    export default {
        data(){
            return{
                ...
                change_po_link  : '/api/update_po_status/'+this.quantity_purchase_data.slack,
                delete_qp_api_link : '/api/delete_quantity_purchase/'+this.quantity_purchase_data.slack,
                generate_invoice_from_po_api_link : '/api/generate_invoice_from_po/'+this.quantity_purchase_data.slack,

                slack           : this.quantity_purchase_data.slack,
                qp_basic        : this.quantity_purchase_data,
                products        : this.quantity_purchase_data.products,

                tax_component_count  : (this.quantity_purchase_data.tax_option_data != null)?this.quantity_purchase_data.tax_option_data.component_count:0,
                tax_component_array  : (this.quantity_purchase_data.tax_option_data != null)?this.quantity_purchase_data.tax_option_data.component_array:[],
                table_colspan   : parseInt(8)+((this.quantity_purchase_data.tax_option_data != null)?parseInt(this.quantity_purchase_data.tax_option_data.component_count):1),
                
                update_stock    : (this.quantity_purchase_data.update_stock == 1)?'Yes':'No',
                block_delete_qp : ['CLOSED', 'RELEASED_TO_SUPPLIER']
            }
        },
        props: {
            quantity_purchase_data: [Array, Object],
            po_statuses: Array,
            delete_qp_access: Boolean,
        ...
        methods: {
            ...
            delete_qp(){
                ...
                this.$on("submit",function () {       
                    ...
                    axios.post(this.delete_qp_api_link, formData).then((response) => {
