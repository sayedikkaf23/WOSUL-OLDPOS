<template>
  <div class="row">
    ...
    <div class="col-md-12">
      ...
      <strong> {{ $t("Hello Dear Merchant,") }} </strong><br>
      {{ $t("We would like to inform you that quantity purchase can be used to add inventory only, it wont be considered as expense to company Instead") }} <br>
      {{ $t("You can use purchase order if you wants to record expense and add inventory both") }} <br>
      </div>
    </div>
          ...
          <div class="row" v-if="products.length">
            ...
            >
              <div
                ...
                v-on:click="addProductToBag(product,product_index)"
              >
          ...
          >
            <div class="col-sm-12 select-product-img-blk">
              <div class="select-product-img">
                <img
                  ...
                  @error="imageUrl = default_product_image_path"
                />
              </div>
            </div>
            <div class="col-sm-12 select-product-price-blk">
              ...
              <div class="row">
                <div class="col-6">
                  <div class="">
                    <!-- <label>Qty: {{ item.product_unit }}</label> -->
                    <label>Price: {{ item.product_price }} {{ session_currency_code }}</label>
                  </div>
                </div>
                <div class="col-6">
        ...
        <div class="card-body">
          ...
          <form class="purchases-form">
            ...
            <button
              ...
              v-if="this.quantity_purchase_data == null"
            >
    ...
    <modalcomponent v-if="show_modal" v-on:close="show_modal = false">
      ...
      <template v-slot:modal-footer>
        ...
        <button type="button" class="btn btn-primary" v-on:click="submitForm()" v-bind:disabled="processing == true">
...
export default {
  data() {
    return {
      ...
      api_link        : (this.quantity_purchase_data == null)?'/api/add_quantity_purchase':'/api/update_quantity_purchase/'+this.quantity_purchase_data.slack,
      product_bag: [],
      product_bag_items: 0,
      total_amount: 0,
      discount_rate: 0,
      discount_amount: 0,
      order_date: (this.quantity_purchase_data != null) ? this.quantity_purchase_data.order_date_raw : new Date().toISOString().substr(0, 10), // dd/mm/yyyy
      show_modal: false,
      ...
      total_tax_amount : 0,
      processing: false,
      default_product_image_path : '/storage/default-product-image.png',
    };
  },
  ...
  props: [
    ...
    "merchant_id",
    "quantity_purchase_data"
  ],

  mounted() {
    /* edit purchase order */

    if(this.quantity_purchase_data != null){

        $("#supplier").val(this.quantity_purchase_data.supplier.slack);
        //this.po_number = this.quantity_purchase_data.po_number;
        this.po_reference = (this.quantity_purchase_data.po_reference != 'null') ? this.quantity_purchase_data.po_reference : '';
        this.discount_type = this.quantity_purchase_data.discount_type;
        this.discount_rate = this.quantity_purchase_data.discount_rate;
        this.terms = this.quantity_purchase_data.terms;

        if(this.quantity_purchase_data.products != null){
  ...
  methods: {
    ...
    loadEditPurchaseOrder(){
      ...
      $.each(this.quantity_purchase_data.products, function(index, product) {

        let item = {};
        
        item['index'] = product.id;
        item['product_image'] = product.product_detail.product_thumb_image;
        ...
        item['product_id'] = product.product_detail.id;
    ...
    addProductToBag(product,index) {
      ...
      let duplicate = this.product_bag.find((p) => p.product_id === item.product_id);
        ...
        .then((response) => {
          ...
          if (response.data.status) {
            if (response.data.status_code == 200) {
              ...
              location.href = "/quantity_purchase/" + response.data.data;
            } else {
              this.processing = false;
