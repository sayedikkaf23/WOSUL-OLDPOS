        ...
        <div class="col-md-12">
            ...
            <div class="d-flex flex-wrap mb-4" v-if="quotation_statuses != ''">
                ...
                <div class="ml-auto">
                    ...
                    <a class="btn btn-outline-success mr-1" v-show="quotation_basic.status.value == 2" v-bind:href="'/invoice/'+quotation_basic.converted_invoice_slack"
                        v-if="quotation_basic.converted_invoice_slack != null && quotation_basic.converted_invoice_slack.length > 0"  target="_blank" >
                        {{ $t("View Converted Invoice") }}
                    </a>
                    <a class="btn btn-outline-warning mr-1" v-show="quotation_basic.status.value == 2" v-else v-on:click="quotation_to_invoice(quotation_to_invoice_link)">
                        {{ $t("Convert to Invoice") }}</a>
                    <a class="btn btn-outline-primary mr-1" v-bind:href="'/print_quotation/'+slack" target="_blank">{{ $t("Print Format") }} 1</a>
                    <a class="btn btn-outline-primary mr-1" v-bind:href="'/print_quotation_alt/'+slack" target="_blank">{{ $t("Print Format") }} 2</a>
                    ...
                    <div class="dropdown d-inline">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="quotation_action">
                            <button class="dropdown-item" type="button" v-for="(quotation_status, key, index) in quotation_statuses" v-bind:value="quotation_status.value_constant" v-bind:key="index" v-on:click="change_quotation_status(quotation_status.value_constant)">{{ $t("Mark as") }} {{ quotation_status.label }}</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-row mb-2">
                <div class="form-group col-md-12">
                    <p class='custom-pre'>{{ (quotation_basic.terms != null)?quotation_basic.terms:'-' }}</p>
                </div>
            </div>
        </div>
    ...
    export default {
        data(){
            return{
                ...
                quotation_to_invoice_link  : '/api/quotation_to_invoice/'+this.quotation_data.slack,
            ...
            quotation_to_invoice(api_url){
                ...
                axios.post(api_url, formData).then((response) => {
                    ...
                    if(response.data.status_code == 200) {
                        ...
                        this.quotation_basic.converted_invoice_slack = response.data.data;
                        // location.reload();
                    }else{
                        try{
                            var error_json = JSON.parse(response.data.msg);
