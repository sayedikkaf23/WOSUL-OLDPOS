        ...
        <div class="col-md-12">
            ...
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="form-row mb-2">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select name="to_store" v-model="to_store" v-validate="'required'" class="form-control form-control-custom custom-select">
                            <option value="">{{ $t("Choose To Store..") }}</option>
                ...
                <div class="form-row mb-2">
                    ...
                    <div class="form-group col-md-3">
                        <label for="barcode">{{ $t("Transfer Type") }} </label>
                        <Select2 v-model="transfer_type" :options="transferTypes" @change="update_transfer_types(transfer_type)" />
                    </div>

                    <div class="form-group col-md-3">
                        <label for="barcode">{{ $t("Search and Add Products") }} <a class="btn btn-sm btn-secondary" href="/add_product" target="_BLANK">+</a> </label>
                        <Select2 v-model="search_product" :options="productOptions" @change="add_product_to_list(search_product)" />
                    </div>
                </div>
                ...
                <div class="form-row mb-2" v-for="(product, index) in products" :key="index">
                    <div class="form-group col-md-5">
                        <input type="text" v-bind:name="'product.name_'+index" v-model="product.name" v-validate="'required|max:250'" :data-vv-as="product_name" class="form-control form-control-custom" autocomplete="off" readonly>
                        <span v-bind:class="{ 'error' : errors.has('product.name_'+index) }">{{ errors.first('product.name_'+index) }}</span> 
                    </div>
                    <div class="form-group col-md-1">
                        <input type="number" v-bind:name="'product.quantity_'+index" v-model="product.quantity" v-validate="'required|decimal|min_value:0.01'" data-vv-as="Quantity" class="form-control form-control-custom"  autocomplete="off" step=".01" min="0">
                    ...
                    <div class="form-group col-md-4">
                        <span class="d-block mt-1" v-if="product.quantity_left != null &&product.quantity_left != ''">{{ $t("Total Stock Left") }}: <span v-if="product.quantity_left == -1">Unlimited</span> <span v-else>{{ product.quantity_left }}</span> </span>
                    </div>
                </div>
                ...
                <div class="form-row mb-2">
                 ...
                 <div class="flex-wrap mb-4">
                    <div class="text-right" v-if="edit_stock_transfer_access == true">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update_transfer_types(transfer_type){
                if(transfer_type == 1){
                    this.productOptions = [];
                    this.all_products.forEach((product, index) => {
                      this.productOptions.push({
                    ...
                    this.productOptions = [];
                    this.all_ingredients.forEach((product, index) => {
                      this.productOptions.push({
                        id: product.slack,
                        text: product.name,
                      });
