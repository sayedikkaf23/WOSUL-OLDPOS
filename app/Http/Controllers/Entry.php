...
class Entry extends Controller
{
    ...
    {   
        ...
        $data['store_logo_path'] = null;
        $store_logo = Store::select('store_logo')->orderBy('id','ASC')->first();
        if(Storage::disk('store')->has($store_logo->store_logo)){
            $data['store_logo_path'] = str_replace('://','://'.$subdomain.'.',Storage::disk('store')->url($store_logo->store_logo));
        }
