        ...
        try {
            ...
            if (!empty($user_exists)) {
                ...
                if (!empty($selected_store)) {
                    ...
                    // $knownDate = Carbon::create(2022, 03, 22, 00, 30, 00);
                    // Carbon::setTestNow($knownDate);

                    // managing store's opening and closing counter
                    $store_time = StoreModel::select('store_opening_time', 'store_closing_time', 'is_store_closing_next_day')->find($user_exists->store_id);
                    if ($store_time->store_opening_time == '' || $store_time->store_closing_time == '') {
                        $opening_time = Carbon::now()->startOfDay();
                        $closing_time = Carbon::now()->endOfDay();
                    } else {
                        $opening_time = Carbon::createFromTimeString($store_time->store_opening_time);
                        $closing_time = Carbon::createFromTimeString($store_time->store_closing_time);

                        if ($store_time->is_store_closing_next_day == 1) {

                            if (Carbon::now()->format('H:i') < $opening_time->format('H:i') && Carbon::now()->format('H:i') < $closing_time->format('H:i')) {
                                $opening_time = $opening_time->subDay();
                                $closing_time = $closing_time;
                            } else if (Carbon::now()->format('H:i') > $closing_time->format('H:i') && Carbon::now()->format('H:i') < $opening_time->format('H:i')) {
                                $opening_time = $opening_time->subDay();
                                $closing_time = $closing_time;
                            } else {
                                $opening_time = $opening_time;
                                $closing_time = $closing_time->addDay();
                            }
                        }
                    }
                    ...
                    // echo $closing_time;
                    // exit;
                    $request->logged_store_opening_time = Carbon::parse($opening_time)->format('Y-m-d H:i:s');
                    $request->logged_store_closing_time = Carbon::parse($closing_time)->format('Y-m-d H:i:s');
                } else {
